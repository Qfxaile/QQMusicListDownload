...
async def convert_to_mp3(
    file_name: str, original_path: str, mp3_path: str, current: int, total: int
):
    print(f"({current}/{total}) 正在转换 {file_name} 为 MP3...")
    audio = AudioSegment.from_file(original_path)  # 读取下载的文件
    audio.export(mp3_path, format="mp3")  # 转换为 mp3 格式
    print(f"({current}/{total}) {file_name}.mp3 转换成功，保存到 {mp3_path}")
    os.remove(original_path)  # 删除原始文件
    print(f"({current}/{total}) 原始文件 {original_path} 已删除")
...
async def download_music(
    ...
    async with semaphore:  # 使用信号量来限制并发数量
        try:
            music_info = await get_music_info(session, songmid, br=6)
            if music_info.get("src"):
                ...
                # 如果 MP3 文件已存在，跳过下载和转换
                if os.path.exists(mp3_path):
                    print(
                        f"({current}/{total}) {file_name}.mp3 已存在，跳过下载和转换。"
                    )
                    return

                print(f"({current}/{total}) 正在下载 {file_name}...")
                # 异步下载音乐文件
                    ...
                    if response.status == 200:
                        with open(music_path, "wb") as f:
                            while True:
                                ...
                                f.write(chunk)
                        print(
                            f"({current}/{total}) {file_name} 下载成功，保存到 {music_path}"
                        )

                        # 并发执行音频转换任务
                        asyncio.create_task(
                            convert_to_mp3(
                                file_name, music_path, mp3_path, current, total
                            )
                        )
                    else:
                        print(
                            f"({current}/{total}) 下载 {file_name} 失败，状态码：{response.status}"
                        )
            else:
                print(f"({current}/{total}) 无法获取下载链接。")

        except (ClientPayloadError, ContentLengthError) as e:
            print(f"({current}/{total}) 下载失败，错误信息：{str(e)}")
            print(f"({current}/{total}) 停止程序执行。")
            # 停止程序
            exit(1)
...
async def main():
    ...
    total_songs = len(music_list)  # 歌曲总数
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for index, music in enumerate(music_list):
            ...
            if song_name and songmid:
                tasks.append(
                    download_music(
                        session, song_name, songmid, semaphore, index + 1, total_songs
                    )
                )
